...
void mn_page_appear(mn_page_t* page)
{
  switch(page->uType&MN_PAGE_TYPE_MASK)
  {
    case MN_PAGE_TYPE_CUSTOM:
    {
      ...
      // if(mn_pcustom->lpfnAppear) mn_pcustom->lpfnAppear(page);
      break;
    }
...
void mn_page_diappear(mn_page_t* page)
{
  ...
  switch(type)
  {
    case MN_PAGE_TYPE_CUSTOM:
    {
      #if MN_LOG_TRACE_PAGE_DISAPPEAR
      MN_LOG_TRACE("CUSTOM");
      #endif

      // mn_pcustom = (mn_p_custom_t*)page;
      // if(mn_pcustom->lpfnDisappear) 
      //   mn_pcustom->lpfnDisappear(page);
      // else
      //   mn_disappear_default(page);
...
void mn_page_handler(mn_page_t* page)
{
  switch(page->uType&MN_PAGE_TYPE_MASK)
  {
    ...
    {
      // mn_pcustom = (mn_p_custom_t*)page;
      // if(mn_pcustom->lpfnHandler) mn_pcustom->lpfnHandler(page);
      break;
    }
    case MN_PAGE_TYPE_LIST:
    {
...
void mn_page_painter(mn_page_t* page)
{
  switch(page->uType&MN_PAGE_TYPE_MASK)
  {
    ...
    {
      // mn_pcustom = (mn_p_custom_t*)page;
      // if(mn_pcustom->lpfnPainter) mn_pcustom->lpfnPainter(page);
      break;
    }
    case MN_PAGE_TYPE_LIST:
    {
...
{
  mn_u8g2 = u8g2;
  u8g2_SetFont(mn_u8g2, mn_defaultParam.puFont);
  u8g2_SetBitmapMode(mn_u8g2,1);

  mn_pointer_init(&mn_pointer1,&mn_defaultParam,1);
  mn_pointer_init(&mn_pointer2,&mn_defaultParam,1);
  ...
  switch(arg_int)
  {
    case 1:
    {
      ...
      for(int i=10;i<96;i+=2)
      {
        ...
        // u8g2_DrawRFrame(mn_u8g2,16-1,40-1,96+2,10+2,1);
        // u8g2_SetDrawColor(mn_u8g2,2);
        // u8g2_DrawRBox(mn_u8g2,16-1,40-1,96+2,10+2,1);
        // u8g2_SetDrawColor(mn_u8g2,0);
        // u8g2_DrawBox(mn_u8g2,16,40,96,10);
